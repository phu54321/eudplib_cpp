...
SimpleArchive::SimpleArchive(std::istream& is)
{
	while(true)
	{
		...
		_contentMap[sectionName] = std::move(data);
	}
}
...
{
	auto it = _contentMap.find(sectionName);
	if(it == _contentMap.end()) return false;
	else
	{
		result = it->second;
		return true;
...
{
	_contentMap[sectionName] = src;
}

bytes SimpleArchive::toBytes() const
{
	size_t payloadSize = 0;
	for(const auto& it : _contentMap)
	{
